...
func StreamPOSSideVoice(
	stream pb.Streaming_StreamPOSSideVoiceServer,
		...
		for {
			...
			default:
				...
				if data != nil && len(data.Data) > 0 {
					broker.Produce(fmt.Sprintf("POSToOp-Voice-%v", talkSessionId), data.Data)
	...
	eg.Go(errUtil.RecoverPanic(func() error {
		dataChannel, errChannel, closeConsumer := broker.Consume(ctx, fmt.Sprintf("OpToPOS-Voice-%v", talkSessionId), true)
		for {
			select {
			case <-ctx.Done():
				err := closeConsumer()
				...
				err = broker.DeleteTopic(fmt.Sprintf("OpToPOS-Voice-%v", talkSessionId))
				if err != nil {
					fmt.Println(err)
				}
				return ctx.Err()
