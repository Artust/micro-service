...
func Create(
	...
	input *pb.CreateAccountRoleRequest,
) (*pb.AccountRole, error) {
	data := entity.AccountRole{}
	...
	defer session.Close()
	newRole, err := session.WriteTransaction(func(tx neo4j.Transaction) (interface{}, error) {
		ctx = context.WithValue(ctx, config.Neo4jTransactionContextKey, tx)
		role, err := accountRoleRepository.Create(ctx, &data)
		if err != nil {
			return nil, err
		}
		return role, nil
	...
	role := newRole.(*entity.AccountRole)
